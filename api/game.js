    ...
    switch (action) {
      ...
      case 'submit_vote': {
        ...
        if (Object.keys(room.votes).length >= room.players.length) {
          computeResults(room);
          const rnd = room.rounds[room.currentRound];
          await pusher.trigger(`room-${code}`, 'results_ready', {
      ...
      case 'end_game': {
        ...
        if (!room) return res.json({ ok: false });
        room.phase = 'gameover';
        await pusher.trigger(`room-${code}`, 'game_over', { scores: room.scores, players: room.players });
        return res.json({ ok: true });
      }
      default:
